  ...
  return ReactDom.createPortal(
    <div className="fixed top-0 left-0 w-screen h-screen z-50">
      ...
      <div className="flex flex-col bg-white absolute right-0 top-0 h-screen shadow-lg w-screen sm:w-96 max-w-screen gap-4">
        ...
        <div className="p-4 overflow-scroll flex-1 flex flex-col gap-4 text-gray-500 text-center">
          {cartItems.length === 0 ? (
            <p>Cart is empty</p>
          ) : (
            <>
        ...
        <div
          onClick={checkout}
          className=" text-xl m-4 p-6 grid place-items-center hover:opacity-60 cursor-pointer"
        >
          Checkout
        </div>
      </div>
